...
function countVowels(str) {
  ...
  return Array.from(str).reduce((res, l) => {
    return vowels.includes(l) ? res + 1 : res;
  }, 0);
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((res, word) => (res.length < word.length ? word : res), '');
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((newStr) => reverseString(newStr))
    .join(' ');
}
...
function invertCase(str) {
  return str
    .split('')
    .map((l) => (l.toLowerCase() === l ? l.toUpperCase() : l.toLowerCase()))
    .join('');
}
...
function extractNameFromTemplate(value) {
  const newStr = value.split(' ').map((word) => word.replace(/[^a-zA-Z]/g, ''));
